...
In `_targets.R`, we call `calc_saidi` whichever target(s) require it, using the feature-enhanced Eagle-I dataset in the preceding target, named `add_features`. See the screengrab below for example function output from the `county_saidi` target.
